...
Those anomalous entries come almost entirely from one day.  Our guess is that the
detection limit for the method used does not vary, and so, these observations
were incorrectly recorded. Luckily, the apparent inconsistencies are very rare,
and can be  adjusted with little concern for loss of accuracy.
```{r}
